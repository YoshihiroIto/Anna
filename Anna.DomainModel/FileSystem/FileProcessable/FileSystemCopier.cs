...
public abstract class FileSystemCopier : IFileProcessable
{
    ...
    public void Invoke()
    {
        ...
        {
            Dic.GetInstance<ILoggerService>().Information($"{nameof(FileSystemCopier)}.{nameof(Invoke)} -- Canceled");
        }
        finally
        {
...
{
    public DefaultFileSystemCopier(IServiceProvider dic, IEntry[] sourceEntries, string destPath, CopyOrMove copyOrMove)
        : base(dic)
    {
